    ...
    <div className="max-w-4xl mx-auto py-12 px-4 space-y-10">
      ...
      <AnimatePresence mode="wait">
        <motion.div
          ...
          className="bg-slate-900 p-6 sm:p-10 md:p-16 rounded-[2rem] sm:rounded-[3rem] md:rounded-[3.5rem] shadow-2xl border border-slate-800 relative overflow-hidden"
        >
          <div className="absolute top-6 sm:top-10 right-6 sm:right-10 opacity-[0.03] select-none pointer-events-none text-white">
            <Shield className="w-32 h-32 sm:w-64 sm:h-64" />
          </div>

          <div className="relative z-10">
            <div className="inline-block px-3 sm:px-4 py-1 bg-slate-950 text-slate-500 rounded-full text-[8px] sm:text-[10px] font-bold uppercase tracking-widest mb-6 sm:mb-10 border border-slate-800">
              Exam Item #{currentQuestionIndex + 1}
            </div>
            
            <div className="text-lg sm:text-2xl md:text-3xl font-bold text-white mb-8 sm:mb-12 font-serif leading-relaxed">
            ...
            <div className="grid grid-cols-1 gap-4 sm:gap-6">
              {currentQuestion?.options.map((option, i) => (
                <button
                  ...
                  className={`group w-full p-4 sm:p-6 md:py-8 rounded-xl sm:rounded-[2rem] border-2 text-left transition-all flex items-center space-x-3 sm:space-x-6 relative overflow-hidden ${
                    answers[currentQuestion.id] === i
                      ? 'border-[#D4AF37] bg-amber-500/5 shadow-lg shadow-amber-900/10'
                ...
                >
                  ...
                  <div className="min-w-0">
                    <MathRenderer content={option} className={`font-bold text-sm sm:text-xl ${answers[currentQuestion.id] === i ? 'text-white' : 'text-slate-300'}`} engine={profile?.mathEngine} />
                  </div>
                </button>
              ))}
            </div>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center gap-6 px-6 pb-20">
        <button
          ...
          className="flex items-center space-x-3 px-8 py-4 rounded-2xl font-bold text-slate-400 hover:bg-slate-900 transition-all disabled:opacity-20 border border-transparent hover:border-slate-800 shadow-sm"
        >
          <ChevronLeft className="w-6 h-6" />
          <span>Previous Task</span>
        </button>

        <div className="flex flex-wrap items-center justify-center gap-2 px-6 py-3 bg-slate-900 rounded-2xl sm:rounded-full max-w-full border border-slate-800">
          {questions.slice(0, 15).map((_, i) => (
            <div 
              key={i} 
              className={`w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full transition-all ${
                i === currentQuestionIndex ? 'bg-[#D4AF37] w-4 sm:w-6 shadow-[0_0_8px_rgba(212,175,55,0.6)]' : 
                answers[questions[i].id] !== undefined ? 'bg-amber-700' : 'bg-slate-800'
              }`} 
            />
          ))}
          {questions.length > 15 && <span className="text-[8px] font-bold text-slate-500">+{questions.length - 15} more</span>}
        </div>
        ...
        {currentQuestionIndex === questions.length - 1 ? (
          ...
          <button
            ...
            className="w-full sm:w-auto bg-[#D4AF37] text-slate-950 px-12 py-5 rounded-[2rem] font-bold text-xl shadow-2xl shadow-amber-900/20 hover:bg-amber-400 transition-all flex items-center justify-center space-x-3 transform hover:-translate-y-1 group"
          >
